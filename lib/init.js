...
module.exports = function $initPubSubMQTT(context, config) {
    ...
    // function _shouldAttemptRecovery(client, error) {
    //     //TODO: Should we track #s of attempted recoveries?
    //     return (
    //         client.disconnecting === true &&
    //         client.disconnecting === client.disconnected
    //     );
    // }

    // pubsub.attempteRecovery = function() {
    //     _logger.error('--- attempteRecovery: request end');
    //     client.end(true, _ => {
    //         _logger.error('--- attempteRecovery: end complete, reconnecting...');
    //         client.reconnect(config.transport);
    //     });
    // };
    ...
    client.on('error', (err) => {
        ...
        _logger.error(Object.keys(err));
        _logger.error('disconnecting: %s - disconnected: %s', client.disconnecting, client.disconnected);

        // if (_shouldAttemptRecovery(client, err)) {
        //     return pubsub.attempteRecovery();
        // }
    ...
    client.on('reconnect', _ => {
        ...
        if (pubsub.connectionAttempt > config.maxConnectionAttempts) {
            if (config.connectionNeeded) {
                ...
                _logger.error('disconnecting: %s - disconnected: %s', client.disconnecting, client.disconnected);
                return client.end(true, _ => {
                    _logger.warn('Client DID end connection.');
                    _logger.error('disconnecting: %s - disconnected: %s', client.disconnecting, client.disconnected);
                });
            }
        }
